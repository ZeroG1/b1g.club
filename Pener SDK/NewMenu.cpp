	...
	ImGui::BeginChild("##tabs2", ImVec2(400, 0), true, 0);
	{
		...
		case 0:
		{
			...
			if (SETTINGS::settings.aim_bool) {
				ImGui::SliderFloat("hitchance", &SETTINGS::settings.chance_val, 0.f, 100.f, ("%1.0f"));
				ImGui::SliderFloat("minimum damage", &SETTINGS::settings.damage_val, 0.f, 100.f, ("%1.0f"));
				ImGui::Combo("aimbot hitbox", &SETTINGS::settings.acc_type, acc_mode, ARRAYSIZE(acc_mode));
				ImGui::Checkbox("resolver", &SETTINGS::settings.resolver_bool);
		...
		case 1:
		{
			...
			if (SETTINGS::settings.esp_bool)
			{
				...
				if (SETTINGS::settings.fov) {
					ImGui::SliderFloat("value", &SETTINGS::settings.FOV, -50, 50, ("%1.0f"));
		...
		case 2:
		{
			...
			if (SETTINGS::settings.misc_bool)
			{
				...
				if (SETTINGS::settings.lag_bool) {
					ImGui::SliderFloat("standing lag", &SETTINGS::settings.stand_lag, 1, 15, ("%1.0f"));
					ImGui::SliderFloat("moving lag", &SETTINGS::settings.move_lag, 1, 15, ("%1.0f"));
					ImGui::SliderFloat("jumping lag", &SETTINGS::settings.jump_lag, 1, 15, ("%1.0f"));
				}
		...
		case 3:
		{
			...
			if (SETTINGS::settings.aa_bool)
			{
				...
				ImGui::SliderFloat("spin speed", &SETTINGS::settings.antiaim.spinspeed, -50, 50, ("%1.0f"));
				ImGui::SliderFloat("jitter range", &SETTINGS::settings.antiaim.jitterrange, -360, 360, ("%1.0f"));
				...
				if (SETTINGS::settings.antiaim.breaklby.enable) {
					...
					ImGui::SliderFloat("lby delta", &SETTINGS::settings.antiaim.breaklby.delta, -180, 180, ("%1.0f"));
				}
		...
		case 4:
		{
			...
			if (ImGui::Button("Load Config", ImVec2(120, 30)))
			{
				...
				GLOBAL::Msg("[b1g.club beta] Configuration loaded.    \n");
			}
			...
			if (ImGui::Button("Save Config", ImVec2(120, 30)))
			{
				...
				GLOBAL::Msg("[b1g.club beta] Configuration saved.    \n");
			}
		}
		break;
		}
