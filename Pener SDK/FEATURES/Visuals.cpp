...
void CVisuals::DrawInfo(SDK::CBaseEntity* entity, CColor color, CColor alt)
{
	...
	if (!using_fake_angles[entity->GetIndex()] && SETTINGS::settings.resolver_bool)
	{
		...
		if (local_player->GetHealth() == 0)
			stored_info.push_back(std::pair<std::string, CColor>("not resolving", color)); //not resolving
		else if (resolve_type[entity->GetIndex()] == 1)
			stored_info.push_back(std::pair<std::string, CColor>("moving", color)); //moving
			...
			stored_info.push_back(std::pair<std::string, CColor>("lby updates", color)); //lby updates
		else if (resolve_type[entity->GetIndex()] == 4)
			stored_info.push_back(std::pair<std::string, CColor>("bruteforce", color)); //bruteforce
			...
			stored_info.push_back(std::pair<std::string, CColor>("lby backtracking", color)); //backtracking lby
		else
...
void CVisuals::Clantag()
{
	...
	if (GetTickCount() > lastTime)
	{
		...
		if (kek > 10) {
			...
			setClanTag(clantag[autism], "b1g.club");
